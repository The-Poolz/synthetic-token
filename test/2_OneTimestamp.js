...
contract("Testing Synthetic Token with one timestamp", accounts => {
    ...
    it('get activation result with one timestamp', async () => {
        ...
        assert.equal(result[2].toString(), timestamp.toString(), 'check start times')
        assert.equal(result[3].toString(), timestamp.toString(), 'check finish times')
    })
    ...
    it('testing get activation in the past time', async () => {
        ...
        assert.equal(result[2].toString(), pastTimestamp, 'check unlock time')
        assert.equal(result[3].toString(), pastTimestamp, 'check unlock amount')
    })

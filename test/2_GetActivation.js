...
contract("Testing getActivationResult", accounts => {
    ...
    it('testing get activation in the past time', async () => {
        ...
        const now = new Date()
        const pastTime = (now.setHours(now.getHours() - 1) / 1000).toFixed()
        await token.SetLockingDetails(testToken.address, pastTime, pastTime, finishTime.toString(), { from: secondAddress })
